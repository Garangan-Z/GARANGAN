...
\033[97;1m[ \033[92;1mFollow Me Facebook And Github \033[97;1m]
\033[92;1m----------------------------------------------------------
""")
	...
	if len(berapa_d)==0:
		...
		for angka in berapa_d:
			...
			if dutt>3599:
				...
				dett = ditt[0]+"d"
	print("\r [Chan] %s/%s OK:%s CP:%s %s[%s%s%s] [%s%s%s]"%(loop,len(id),len(ok),len(cp),h,k,persen,h,k,dett,h), end=' ');sys.stdout.flush()
